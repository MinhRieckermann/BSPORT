...
namespace WebAPI_Team.Services.OddAnalysisService
{
    public class OddAnalysisService
    {
        ...
        // Get  with Specifing 2 parameter
        public JsonObject GetListOddAnalysis_Country_week(string country,string Week)
        {
            try
            {

                var ListOddAnalysis = _unitOfWork.OddAnalysisRepository.Get(x => x.country.Equals(country) && x.Week.Equals(Week));
                var total = ListOddAnalysis.Count();
                JsonObject json = new JsonObject()
                {
                    objects = ListOddAnalysis,
